...
function NotebookContent({ selectedDate, selectedGroup, students }) {
    // Initialize state from localStorage. Because of the key on the parent, 
    // this component remounts when date/group changes, so this runs every time we switch context.
    ...
    // Expose current data for export via a ref or callback if needed, 
    // but here we can just pass the data to the export function if it was inside.
    // Since export is in parent, we might need to lift state or move export here.
    // Let's move export here.
    ...
    return (
        <div className="animate-fade-in pb-20">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-md border border-gray-200 dark:border-gray-700">
                ...
                {/* Content - Remounts when date/group changes */}
                <NotebookContent
                    key={`${selectedDate}-${selectedGroup}`}
